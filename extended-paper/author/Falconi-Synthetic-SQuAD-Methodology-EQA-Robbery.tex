...
This paper extends the earlier study by treating data quality and answerability as central experimental variables. It generates candidate SQuAD v2-style instances using open LLMs, applies reproducible structural quality controls, conducts a stratified artificial audit, and establishes a context-disjoint, LLM-audited evaluation subset. The contributions are as follows:
\begin{enumerate}
...
\item A stratified artificial audit (LLM-as-judge) that distinguishes structural span validity from semantic QA correctness.
\item A reproducible evaluation protocol that separates raw synthetic data, quality-filtered training data, and an LLM-audited gold test set.
\end{enumerate}
...
\subsubsection{Models for SQuAD v2 Dataset Generation and Audit}
...
The stratified semantic audit is performed by GLM-5.3, an LLM acting as an
artificial reviewer (reviewer id \texttt{computer-v1})~\cite{glm_reviewer_hf,zeng2024chatglm}.
The GLM family is a series of bilingual Chinese--English large language models
developed by Zhipu AI; the specific checkpoint used as reviewer must be
confirmed, but the family report describes models with long-context support
and tool-use capabilities that make them suitable for structured annotation
review.
...
% Source: Hugging Face model cards (URLs below); short names defined in this chapter.
\begin{table}[!t]
...
GLM-5.3 & \texttt{(to be confirmed)} & \seqsplit{https://huggingface.co/zai-org} \\
...
GLM-5.3 & up to 128K & Yes (bilingual focus) & Limited & No & Rivals GPT-4 on general benchmarks (family report) \\
...
Accordingly, this research does not only ask whether a fine-tuned transformer can obtain a high F1 score on generated annotations. It evaluates whether the generated annotations themselves are reliable enough for training and whether a filtered, quality-controlled dataset produces more trustworthy results on an LLM-audited test set. This transforms the extension into a reproducible investigation of synthetic-data reliability for Legal NLP.
...
(QC) stage; (iii) a stratified semantic audit performed by an LLM
acting as an artificial reviewer; and (iv) a leakage-free,
...
were selected because they are open-weight, can run on a single Nvidia
A16 GPU, and are representative of two different families and model
sizes, which allows the study to compare generator behaviour rather
than relying on a single annotation source. Despite our effort to
process the 174594 reports in the dataset, given time and resource
limits. each model processed the
same 20{,}000-context samples and, for every context, received the five
target information questions shown in the original study (date, time,
...
The automatic QC procedure is deterministic and precedes the semantic audit.
It verifies that required fields are present, that every answerable
\texttt{answer\_text} occurs literally in its \texttt{context}, and that a
recomputed \texttt{answer\_start} points to the same substring. It also
identifies duplicate or ambiguous occurrences and records a transparent
\texttt{qc\_status} (literal, normalized, no-answer, or hallucinated) together
with a reason code.
...
A stratified audit sample of 200 rows (100 per generator) is drawn across
...
by an LLM acting as an artificial reviewer (GLM-5.3, reviewer
id \texttt{computer-v1})~\cite{glm_reviewer_hf}; no human intervention has
...
The audit is a statistically designed estimate of semantic label reliability
and a mechanism for improving automatic filters; it is not intended to
exhaustively label the entire corpus. Because the current audit uses a single
artificial reviewer, inter-annotator agreement metrics are not yet applicable;
confidence intervals for the observed correctness rates are reported instead.
The 200 labeled rows constitute the gold evaluation subset, which is held out
and never used for training, prompt revision, or hyperparameter selection.
...
rows, 8{,}012 validation rows, 8{,}109 test rows, and 200 gold-test rows, all
...
same test and gold splits, and none is treated as a privileged headline model.

Zero-shot evaluation applies the pre-trained checkpoint directly to the test
and gold sets. Fine-tuning adapts each checkpoint to the merged M2 training
...
training. Models are trained for up to 15 epochs with early stopping on
validation F1 (patience 2--3), and the best checkpoint is selected on the
held-out validation split, never on the test set.
...
objects, value). The main definitions are given below.

\begin{definition}[Exact Match]
For a prediction $p$ and a gold answer $g$, EM is 1 if the normalized
prediction equals the normalized gold answer and 0 otherwise.
\end{definition}

\begin{definition}[Token-level F1]
For a prediction $p$ and a gold answer $g$, token-level F1 is
$2 \cdot \frac{P \cdot R}{P + R}$, where $P$ is the fraction of predicted
tokens that appear in the gold answer and $R$ is the fraction of gold tokens
that appear in the prediction. For multiple gold answers, the maximum F1 is
used.
\end{definition}
...
is one where the predicted span matches the gold span.
\end{definition}
...
The GLM-5.3 artificial reviewer audited 200 stratified rows (100 per
generator). Table~\ref{tab:audit-summary} reports the per-generator results:
...
Figure~\ref{fig:audit-accuracy-model} shows the same comparison graphically.
The audit accuracy varies strongly by question type
(Table~\ref{tab:audit-kinds}): Qwen is markedly more accurate on date
...
\begin{table}[!t]
\caption{Semantic audit summary by generator model.}
\label{tab:audit-summary}
\centering
\small
\begin{tabular}{lrrrr}
\toprule
Model & Audited $n$ & Correct & Incorrect & Accuracy \\
\midrule
...
\caption{GLM audit accuracy by generator model (error bars show 95\% Wilson confidence intervals).}
\label{fig:audit-accuracy-model}
\end{figure}

% Source: Reports/audit_glm_v1/outputs/table_audit_by_question_kind.tex
\begin{table}[!t]
\caption{Semantic audit accuracy by question type and generator model.}
\label{tab:audit-kinds}
\centering
\small
\begin{tabular}{llrrr}
\toprule
Model & Question type & Audited $n$ & Correct & Accuracy \\
\midrule
...
Table~\ref{tab:audit-errors} and Figure~\ref{fig:audit-error-types}.
False abstentions are the dominant failure mode (29 cases, 14.5\% of the
audited rows), followed by wrong time references (12 cases, 6.0\%) and
...
\begin{table}[!t]
\caption{Distribution of semantic error types in the audit sample.}
\label{tab:audit-errors}
\centering
\small
\begin{tabular}{lrr}
\toprule
...
\caption{Distribution of semantic error types identified by the GLM artificial reviewer.}
\label{fig:audit-error-types}
\end{figure}

\subsection{EQA Evaluation on the LLM-Audited Gold Set}
...
(8{,}109 rows) and on the LLM-audited gold subset (200 rows). Metrics are
...
The same ordering is observed on the LLM-audited gold subset. Fine-tuning
raises the gold F1 of deepset XLM-Roberta from 0.5093 to 0.6800 and that of
...
gold F1 (0.3201 to 0.2836), which suggests that the distilled encoder does not
benefit from the additional training signal when the reference labels are
semantically audited rather than generated by the same process.
...
% fractions; the test set has 8,109 rows and the gold subset 200 rows.
% Source: out_experiments/option_b_abblation/
\begin{table}[!t]
\caption{Zero-shot and fine-tuned EQA performance of the four encoders on the test and LLM-audited gold sets.}
\label{tab:model-results}
\centering
\small
\begin{tabular}{llrrrrrr}
\toprule
Model & Mode & Test EM & Test F1 & Test HasAns F1 & Test NoAns F1 & Gold EM & Gold F1 \\
\midrule
...
\caption{Comparison of zero-shot and fine-tuned token-level F1 of the four extractive QA encoders on the held-out test set (left) and on the LLM-audited gold subset (right).}
...
% gold_audit_metrics.json). Test set = 457 legacy rows; gold = 200 M2 audit rows.
\begin{table}[!t]
...
Fine-tuned on gold audit & 0.2100 & 0.5925 & 200 & M2 LLM-audited gold rows \\
\bottomrule
\end{tabular}
\end{table}
...
legacy-trained model is evaluated on the M2 LLM-audited gold subset, its F1
