...
Lorena Isabel Barona López\orcidID{0000-0002-5184-3759}}
...
Robbery is a high-volume crime in Ecuador: the Fiscal\'ia General del Estado recorded 67{,}705 reported robberies in 2024, approximately 376 per 100{,}000 inhabitants~\cite{falconi2026robberyqa}. Extractive question answering (EQA) can support investigations, insurance claims, and public-safety planning by returning evidence-backed facts---the location and time of an incident, the stolen objects, and the reported value---directly from narrative reports. EQA is appropriate for this setting because it constrains the output to a contiguous span of the provided report, so every returned answer is traceable to the source text rather than being composed token by token.

Our prior work demonstrated the feasibility of this approach: a Spanish BERT-based EQA model fine-tuned on a generatively constructed SQuAD-style dataset improved the reported F1 score from 81.70 to 82.88~\cite{falconi2026robberyqa}. That proof of concept left three questions open: whether the synthesized supervision was semantically reliable, whether the evaluation protocol was free of data leakage, and whether the result generalizes across encoder families. This study addresses exactly those gaps.

The underlying concern is that a generated answer can be fluent, syntactically valid, and even occur literally in the source text while failing to answer the intended question. A distance may be returned as a location, a time associated with parking a vehicle may be confused with the time of the robbery, or a generic phrase may be selected despite the absence of an informative answer. Literal string matching and a correct character offset are therefore necessary but not sufficient conditions for reliable synthetic EQA supervision. This thesis connects the quality-control stage, the LLM-based semantic review, and the evaluation results throughout the study.

The study is organized around four research questions:
\begin{itemize}
\item \textbf{RQ1.} Can open LLMs construct usable SQuAD v2-style EQA supervision from unlabeled Spanish robbery reports at scale, without per-example human authoring?
\item \textbf{RQ2.} Are literal-span validity and correct character offsets sufficient evidence that a generated question--answer pair is semantically correct, or is a further, independent review step required to detect errors that string matching cannot see?
\item \textbf{RQ3.} Does the constructed resource support effective extractive QA fine-tuning across encoder families and capacities, when evaluated under a leakage-free, context-disjoint protocol and against an independent, LLM-reviewed evaluation subset?
\item \textbf{RQ4.} Under a corrected, leakage-free evaluation protocol, does the original study's headline result still hold, and what specific factors---dataset composition, split methodology, and metric implementation---explain any discrepancy?
\end{itemize}

The main contributions of this study are:
\begin{enumerate}
\item \textbf{An LLM-based methodology for synthetic SQuAD v2 construction} from Spanish robbery narratives, producing answerable and unanswerable EQA instances with answer-span offsets and generator provenance.
\item \textbf{A reproducible quality-control protocol} (structural integrity $\rightarrow$ extractive validity $\rightarrow$ abstention consistency $\rightarrow$ span distribution/usefulness $\rightarrow$ inter-generator agreement) that filters raw LLM output into \texttt{strict}, \texttt{expanded}, and \texttt{merged} dataset variants before any human or LLM review is needed.
\item \textbf{An LLM-based reviewer protocol for semantic reliability assessment}, using a second, independent LLM as a simulated annotator over a stratified sample, with an explicit error typology (wrong semantic type, wrong event reference, partial location, incomplete object list, answered-when-unanswerable, incorrect impossible, false abstention, other) and Wilson-score confidence intervals, explicitly distinguishing structural validity from semantic correctness.
\item \textbf{A leakage-aware evaluation protocol} using context-disjoint train/validation/test partitions and a held-out, LLM-reviewed evaluation subset, correcting the row-level, test-set-leaking split used in the original proof-of-concept study.
\item \textbf{A four-model extractive QA ablation} (two Spanish BETO variants, a multilingual XLM-Roberta model, and a distilled BETO model) comparing zero-shot and fine-tuned performance, including training-dynamics diagnostics (overfitting behavior, early stopping, no-answer threshold calibration) that the original study did not report.
\item \textbf{A direct, controlled replication of the original study's result}, run under the new codebase, that reproduces its headline number and then shows, on the same trained model, why that number does not hold under the new, harder, leakage-free benchmark---turning the original result into a diagnosed baseline rather than a competitor to be beaten.
\end{enumerate}

Compared with the original paper, this study differs in the title and abstract, the dataset (SQuAD v2 with explicit unanswerability versus an all-answerable SQuAD-style resource), the generation stage (two generators with quality control versus a single unverified generator), the split methodology (context-level and leakage-free versus row-level), the number of models evaluated (four encoders versus one), and the evaluation depth (semantic review, threshold calibration, training-dynamics diagnostics, and a controlled replication).
...
Large language models can produce candidate labels, instructions, summaries,
and QA pairs at a scale that is difficult to achieve through exclusive manual
annotation~\cite{zhou2017neuralqg,sun2018answerfocused}. This capability is
...
Hallucination and unfaithfulness refer to generated content that is unsupported
...
(QC) stage; (iii) a stratified LLM-based semantic review performed by an LLM
...
It is organised in five phases that mirror the QC notebook
(\texttt{qc\_squadv2\_datasets.ipynb}):
...
% Source: Hugging Face model cards (URLs below); short names defined in this paper.
\begin{table}[!t]
...
The evaluation follows SQuAD v2 conventions. Exact match (EM) and token-level
F1 are computed with the standard normalization (lowercasing, punctuation
removal, and whitespace collapsing). Metrics are disaggregated by
answerability (HasAns and NoAns) and by question type (date, time, place,
objects, value). The formal definitions of EM, token-level F1, HasAns/NoAns
metrics, and the Wilson score interval are given in Section~4.3, together with
the no-answer threshold protocol.
...
The evaluation uses the following formal metrics.

\begin{definition}[Exact Match]
For a prediction $p$ and a reference answer $g$, EM is 1 if the normalized
prediction equals the normalized reference answer and 0 otherwise.
\end{definition}

\begin{definition}[Token-level F1]
For a prediction $p$ and a reference answer $g$, token-level F1 is
$2 \cdot \frac{P \cdot R}{P + R}$, where $P$ is the fraction of predicted
tokens that appear in the reference answer and $R$ is the fraction of
reference tokens that appear in the prediction. For multiple reference
answers, the maximum F1 is used.
\end{definition}

\begin{definition}[HasAns and NoAns metrics]
HasAns metrics are computed only over answerable questions; NoAns metrics are
computed only over unanswerable questions. A correct NoAns prediction is one
where the model abstains (empty prediction), and a correct HasAns prediction
is one where the predicted span matches the reference span.
\end{definition}

\begin{definition}[Wilson score interval]
The Wilson score interval is a confidence interval for a binomial proportion
$p$: \(\frac{1}{1+\frac{z^2}{n}}\left(p + \frac{z^2}{2n} \pm z
\sqrt{\frac{p(1-p)}{n} + \frac{z^2}{4n^2}}\right)\), where $n$ is the sample
size and $z$ is the standard normal quantile (1.96 for 95\% confidence).
\end{definition}
...
Table~\ref{tab:model-results} reports the zero-shot (ZSL) and fine-tuned (FT)
performance of the four extractive QA encoders on the held-out test set
(8{,}109 rows) and on the LLM-reviewed subset (200 rows). Metrics are
computed with the standard SQuAD v2 normalization and are reported as
fractions; the no-answer threshold used for the test set is the one tuned on
the validation split and frozen before evaluation. Figure~\ref{fig:ablation-comparison}
provides a graphical comparison of the same results.
...
\subsection{Paper Replication Results}
...
\begin{table}[!t]
\caption{Paper replication results of the previous work on the legacy all-answerable dataset.}
...
The proposed workflow preserves the benefit of LLMs---the ability to generate large pools of candidate QA pairs from unlabeled documents---while adding controls appropriate for legal NLP. Automatic checks ensure reproducible minimum requirements. The semantic review quantifies residual error and identifies failure modes that can inform stricter filters and improved prompting. An LLM-reviewed evaluation subset prevents final performance claims from depending entirely on labels generated by the same process used to create training data.
...
This study set out to answer four research questions.

\textbf{RQ1} asked whether open LLMs can construct usable SQuAD v2-style EQA
supervision from unlabeled Spanish robbery reports at scale. The answer is
yes, with controls: two open models generated 100{,}000 candidate rows per
generator from 20{,}000 contexts with 100\% structural completeness, and the
quality-control pipeline reduced these to five reproducible dataset variants,
including an 80{,}726-row merged corpus.

\textbf{RQ2} asked whether literal-span validity and correct offsets are
sufficient evidence of semantic correctness. The answer is no. The independent
reviewer pass found that approximately one third of the reviewed rows were
semantically incorrect despite passing structural checks, with false
abstentions and wrong time references as the dominant errors. A literal span
match is therefore necessary but not sufficient for reliable synthetic EQA
supervision.

\textbf{RQ3} asked whether the resource supports effective fine-tuning across
encoder families under a leakage-free protocol. The answer is yes, with clear
differences across capacities: fine-tuning improved all four encoders, with
XLM-Roberta reaching the highest test F1 (0.7801), while the distilled encoder
remained weak, and place and object questions were consistently the most
difficult.

\textbf{RQ4} asked whether the original headline result survives a corrected
protocol. The answer is no, and the reasons are now identifiable: the original
all-answerable test set, the use of the test split for model selection, and a
metric implementation gap. The replication reproduces the original score on
the original protocol and shows the drop under the corrected protocol,
turning the original result into a diagnosed baseline.

The take-home message is that a literal string match and a correct character
offset are necessary but not sufficient conditions for reliable synthetic EQA
supervision. Practitioners and future researchers should reuse four portable
artifacts: the deterministic quality-control pipeline, the LLM-based reviewer
protocol with its error typology, the context-disjoint split methodology, and
the threshold-calibration protocol. Immediate future work should target the
weakest question types (place and objects), scale the reviewed sample toward
approximately 385 rows for a $\pm 5\%$ margin at 95\% confidence, and validate
the reviewer itself against human judgment on a small sub-sample, since the
LLM-reviewed subset is not a substitute for human-verified ground truth.
