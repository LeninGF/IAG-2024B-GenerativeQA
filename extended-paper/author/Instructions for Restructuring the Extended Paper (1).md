...
## 7. Proposed paper structure (approved 2026-09-01)

Follow this structure exactly. It was proposed to the author as a full reorganization, section by section, mapped against everything already in the current draft, and explicitly approved before this instruction file was updated. The changes are organizational and didactic — **primarily affecting the Introduction (trimmed reuse) and Theoretical Background (restructured to be concept-first, then literature-second)**. Materials and Methods and Results keep the same content and depth as previously specified in this document; nothing measured, computed, or reported there is to be cut.

> **Instruction to the rewriting agent:** migrate every existing methodological and results detail from the current draft's §3.1–3.8 (Materials and Methods) and §4.1–4.4 (Results) into the equivalent numbered sections below without omission — same figures, same tables, same protocol descriptions, same level of detail. Only Sections 1 and 2 (Introduction and Theoretical Background) are being reorganized/rewritten; treat Sections 3–6 below as a renumbering and light reordering of existing content, not a rewrite. If any methodological or results detail does not have an obvious home in the new structure, flag it rather than dropping it.

### 1. Introduction
- Open with a **short, condensed** version of the original paper's motivation — robbery statistics from Ecuador's Fiscalía General del Estado (67,705 reported incidents in 2024, ≈376 per 100,000 inhabitants — cf. original paper §1) and the value of EQA for investigations, insurance claims, and public-safety planning. Keep this to 2–3 sentences, not a full reused paragraph — the author has confirmed the reuse budget should stay short and this is the main place reuse would otherwise accumulate.
...
### 2. Theoretical Background — concept-first, then literature-second

This section is restructured relative to the current draft's §2.1–2.5, which mixes definitions and literature review in the same subsections. The new version separates **foundational concepts** (short, numbered, definitional) from **related work** (literature only), and adds one new subsection that was previously missing: a definitional grounding for using an LLM as an automated reviewer, which is central to the methodology but was never introduced as a concept before being used.

**2.1 Foundational Concepts**
- **2.1.1 Language Models, Transformers, and Generative AI.** Keep the existing next-token-prediction formalism and the \(p_{model}(x) \approx p_{data}(x)\) generative-AI framing *(from current §2.1)*. Keep brief — this is background, not a contribution.
- **2.1.2 Extractive Question Answering and the SQuAD v2 Task.** Define span-prediction EQA, the context/question/answer schema, and the SQuAD v2 unanswerability convention (`is_impossible`) *(from current §2.1's EQA material)*. Define EM/F1 only conceptually here ("F1 measures token overlap between predicted and reference spans"); the formal equations and the Wilson-interval/threshold-calibration details stay in Materials and Methods §3.7, where they are actually used — do not duplicate the equations in both places.
- **2.1.3 Zero-Shot and Transfer Learning.** Catastrophic forgetting, the ZSL paradigm, why it matters for evaluating pretrained QA encoders before fine-tuning *(from current §2.2)*.
- **2.1.4 LLMs as Synthetic Data Generators and Reviewers.** New subsection. Define "LLM-as-annotator" and "LLM-as-judge" as established concepts, each with at least one supporting citation, before the paper later uses two LLMs as generators and a third as a reviewer. This is the definitional anchor the current draft is missing — right now the reviewer step appears in Methods with no prior grounding in the literature.

**2.2 Related Work**
- **2.2.1 Transformers in Legal and Evidence-Based NLP.** *(from current §2.2's legal-NLP literature and the relevant parts of current §2.3)* — literature only, no definitions.
- **2.2.2 LLM-Based Synthetic Data Generation for QA.** *(from current §2.3)*.
- **2.2.3 Quality Control and Reliability of LLM-Generated Data.** New/expanded subsection. The hallucination-survey references already in the bibliography (Ji et al. [13], Zhang et al. [29]) are currently used only as generic LLM background; move them here as direct support for why a deterministic QC pipeline and an independent LLM review step are necessary — this ties the literature review directly to the paper's core methodological contribution instead of leaving it as generic context.

**2.3 Research Gap and Paper Positioning** *(from current §2.5, rephrased against the four research questions in §5 above rather than as a standalone summary paragraph)*.

**Note:** "Models Used in This Research" (current §2.4, with Tables 1–2) is removed entirely from Theoretical Background — it describes the experimental setup, not background literature, and belongs in Materials and Methods §3.6 (see below), grouped with the fine-tuning ablation it supports.
...
## 9. Reuse budget (≤30% of the original paper, kept short — confirmed with the author)

Springer requires **at least 50% new material**; capping reuse at 30% of the original paper's text is comfortably inside that requirement. The author has confirmed the intent is to keep reuse on the short side of that cap, not to treat 30% as a target — when in doubt, condense rather than reuse verbatim. Confirm the final figure quantitatively before submission rather than assuming: after the rewrite, run a rough overlap check (e.g., shared-sentence or shared-paragraph count) between the new manuscript and the original paper's ~11 published pages, and confirm reused text is under ~30% of the *original paper's* length (not 30% of the new, longer manuscript — those are different denominators and the guideline is ambiguous about which one is meant, so satisfying the stricter reading is safer).

Content that may draw on the original paper, but should be condensed rather than reused near-verbatim:
- The robbery-statistics motivation paragraph (Introduction) — per §7 above, reduce this to 2–3 sentences rather than reusing the full original paragraph.
- The corpus description numbers (174,594 reports, word-count filtering criteria, mean/median word counts) — the numbers themselves are safe to reuse (they describe an unchanged corpus), but rephrase the surrounding sentences rather than copying them.
- Model-card-style descriptions of the shared models (BETO, mrm8488, XLM-R) where architecture facts are unchanged — keep to short factual restatements, not the original phrasing.
